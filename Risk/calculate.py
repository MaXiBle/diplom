...
def calculate_probabilities(data):
    ...
    for entry in data:
        ...
        total_base_scores = []  # Для хранения всех BaseScore, связанных с данной техникой
                        ...
                        if base_score:
                            try:
                                ...
                                if base_score == "Unknown":
                                    base_score = 1.000001
                                total_base_scores.append(float(base_score))
                            except ValueError:
                                pass  # Если преобразовать не удается, пропускаем это значение

        # Вычисляем total_score для всех CWE
        total_score = sum(total_base_scores) if total_base_scores else 0

        # Если у нас есть BaseScores, то продолжаем вычисления
        if total_base_scores:
            for mapping in taxonomy_mappings:
                infos = mapping.get("INFO", [])
                for info in infos:
                    related_weaknesses = info.get("Related_Weaknesses", [])
                    for weakness in related_weaknesses:
                        ...
                        # Для каждой уязвимости из наблюдаемых примеров
                        for example in observed_examples:
                            impact = example.get("Impact", {})
                            base_score = impact.get("BaseScore")
                            ...
                            if base_score:
                                try:
                                    ...
                                    # Вероятность для этой уязвимости
                                    probability = base_score / total_score if total_score > 0 else 0
                                    ...
                                    technique_probabilities[entry_id].append({
                                        ...
                                        "VectorString": impact.get("VectorString", "")
                                    })
